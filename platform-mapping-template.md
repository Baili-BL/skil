...
<link
  href="https://fonts.bunny.net/css2?family={{fonts-bunny-url-params}}&display=swap"
  rel="stylesheet"
/>
```
...
如需自动亮/暗模式切换，请在 Asset Catalog（`Colors.xcassets`）中使用"Any, Dark"外观，而非硬编码十六进制值。上方的 hex 扩展用于原型制作或单模式场景。
...
使用 `@Environment(\.colorScheme)` 进行模式检测。在生产应用中，建议在 Asset Catalog 中为所有颜色定义"Any, Dark"外观以实现自动切换。上方的 Color 扩展提供单模式十六进制值；在生产环境中替换为 Asset Catalog 引用。
...
通过 HTML `<head>` 中的 Fonts Bunny `<link>` 标签加载字体（参见第 1 节），或通过 `@fontsource` 包：
...
将第 1 节中的 `:root` CSS 自定义属性写入全局样式文件（`globals.css` 或 `index.css`）。Tailwind 配置通过 `var(--token-name)` 引用这些属性，从而自动支持暗色模式。
